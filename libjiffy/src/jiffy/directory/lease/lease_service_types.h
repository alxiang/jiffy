...
class rpc_lease_ack {
 public:
  ...
  rpc_lease_ack(const rpc_lease_ack&) noexcept;
  rpc_lease_ack& operator=(const rpc_lease_ack&) noexcept;
  rpc_lease_ack() noexcept
                : renewed(0),
                  lease_period_ms(0) {
  }

  virtual ~rpc_lease_ack() noexcept;
  int64_t renewed;
...
class lease_service_exception : public ::apache::thrift::TException {
 public:
  ...
  lease_service_exception() noexcept
                          : msg() {
  }

  virtual ~lease_service_exception() noexcept;
  std::string msg;
