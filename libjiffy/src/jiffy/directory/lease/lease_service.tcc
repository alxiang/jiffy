...
template <class Protocol_>
::std::shared_ptr< ::apache::thrift::TProcessor > lease_serviceProcessorFactoryT<Protocol_>::getProcessor(const ::apache::thrift::TConnectionInfo& connInfo) {
  ::apache::thrift::ReleaseHandler< lease_serviceIfFactory > cleanup(handlerFactory_);
  ::std::shared_ptr< lease_serviceIf > handler(handlerFactory_->getHandler(connInfo), cleanup);
  ::std::shared_ptr< ::apache::thrift::TProcessor > processor(new lease_serviceProcessorT<Protocol_>(handler));
  return processor;
}
