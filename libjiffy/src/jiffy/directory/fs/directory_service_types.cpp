...
const std::map<int, const char*> _rpc_perm_options_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(3, _krpc_perm_optionsValues, _krpc_perm_optionsNames), ::apache::thrift::TEnumIterator(-1, nullptr, nullptr));
...
std::string to_string(const rpc_perm_options val) {
  std::map<int, const char*>::const_iterator it = _rpc_perm_options_VALUES_TO_NAMES.find(val);
  if (it != _rpc_perm_options_VALUES_TO_NAMES.end()) {
...
const std::map<int, const char*> _rpc_file_type_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(3, _krpc_file_typeValues, _krpc_file_typeNames), ::apache::thrift::TEnumIterator(-1, nullptr, nullptr));
...
std::string to_string(const rpc_file_type val) {
  std::map<int, const char*>::const_iterator it = _rpc_file_type_VALUES_TO_NAMES.find(val);
  if (it != _rpc_file_type_VALUES_TO_NAMES.end()) {
    return std::string(it->second);
  } else {
...
const std::map<int, const char*> _rpc_storage_mode_VALUES_TO_NAMES(::apache::thrift::TEnumIterator(3, _krpc_storage_modeValues, _krpc_storage_modeNames), ::apache::thrift::TEnumIterator(-1, nullptr, nullptr));
...
std::string to_string(const rpc_storage_mode val) {
  std::map<int, const char*>::const_iterator it = _rpc_storage_mode_VALUES_TO_NAMES.find(val);
  if (it != _rpc_storage_mode_VALUES_TO_NAMES.end()) {
    return std::string(it->second);
  } else {
...
rpc_replica_chain::~rpc_replica_chain() noexcept {
}
...
rpc_file_status::~rpc_file_status() noexcept {
}
...
rpc_file_status::rpc_file_status(const rpc_file_status& other10) noexcept {
  type = other10.type;
  permissions = other10.permissions;
  last_write_time = other10.last_write_time;
}
rpc_file_status& rpc_file_status::operator=(const rpc_file_status& other11) noexcept {
  type = other11.type;
...
rpc_data_status::~rpc_data_status() noexcept {
}
...
rpc_dir_entry::~rpc_dir_entry() noexcept {
}
...
directory_service_exception::~directory_service_exception() noexcept {
}
...
const char* directory_service_exception::what() const noexcept {
  try {
    std::stringstream ss;
